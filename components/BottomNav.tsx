...
export default function BottomNav() {
  ...
  const navItems = [
    ...
    { id: 'scanner', label: 'Check-in', icon: QrCode, path: '/scanner', isAction: true },
    { id: 'analytics', label: 'Stats', icon: BarChart3, path: '/analytics' },
    { id: 'profile', label: 'Profile', icon: User, path: '/profile' },
  ];

  return (
    <View className="absolute bottom-0 left-0 right-0 overflow-hidden" style={styles.navContainer}>
      <BlurView
      ...
      <View 
        ...
        style={{ backgroundColor: isDark ? 'rgba(15, 23, 42, 0.7)' : 'rgba(255, 255, 255, 0.7)' }}
      >
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              />
              <Text className={`text-[10px] font-bold mt-1 uppercase tracking-widest ${isActive ? (isDark ? 'text-emerald-500' : 'text-primary') : 'text-slate-400 dark:text-slate-500'}`}>
                {item.label}
              </Text>
            </TouchableOpacity>
          );
