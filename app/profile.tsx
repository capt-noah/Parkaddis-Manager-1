...
export default function Profile() {
  ...
  const handleDownloadReport = async (title: string) => {
    ...
    try {
      ...
      const csvHeader = 'ID,Plate Number,Status,Payment Status,Duration,Accrued\n';
  ...
  return (
    <SafeAreaView className="flex-1 bg-background dark:bg-slate-900" edges={['top']}>
      ...
      <TopBar 
        showLogo={true}
        profileIcon={<Settings size={22} color={isDark ? "#94a3b8" : "#475569"} />}
        ...
        >
          ...
          <View className="items-center mt-6 mb-10">
            <View className="relative">
              <View className="w-28 h-28 rounded-[40px] overflow-hidden border-4 border-white dark:border-slate-800 shadow-xl shadow-slate-200 dark:shadow-black/50">
                <Image
            ...
            <View className="items-center mt-4">
              <Text className="text-2xl font-headline font-bold text-slate-900 dark:text-white">{user?.fullName || 'Manager'}</Text>
              <Text className="text-sm font-medium text-slate-500 dark:text-slate-400 mt-1 uppercase tracking-widest">{user?.role === 'admin' ? 'Terminal Manager' : 'Staff Member'}</Text>
              <Text className="text-[10px] text-slate-400 mt-1 font-bold">{user?.email}</Text>
            </View>
          </View>
          ...
          <View className="gap-8 w-full">
            <View>
              <Text className="text-[10px] font-headline font-extrabold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 mb-4 px-2">Shift History</Text>
              <View className="bg-white dark:bg-slate-800 rounded-3xl overflow-hidden shadow-sm border border-slate-100 dark:border-slate-700">
                <TouchableOpacity className="flex-row items-center justify-between p-5 active:bg-slate-50 dark:active:bg-slate-700">
                  <View className="flex-row items-center gap-4">
                    ...
                    <View>
                      <Text className="text-sm font-bold text-slate-900 dark:text-white">Morning Shift • Terminal A</Text>
                      <Text className="text-[10px] text-slate-400 dark:text-slate-500 font-medium uppercase tracking-wide">Oct 24, 2023 • 08:00 - 16:00</Text>
                    </View>
                  </View>
            ...
            <View>
              <Text className="text-[10px] font-headline font-extrabold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 mb-4 px-2">Station Reports</Text>
              <View className="bg-white dark:bg-slate-800 rounded-3xl overflow-hidden shadow-sm border border-slate-100 dark:border-slate-700">
                <TouchableOpacity 
                ...
                >
                  <View className="flex-row items-center gap-4">
                    ...
                    <View>
                      <Text className="text-sm font-bold text-slate-900 dark:text-white">Daily Occupancy Summary</Text>
                      <Text className="text-[10px] text-slate-400 dark:text-slate-500 font-medium uppercase tracking-wide">PDF Report • 2.4 MB</Text>
                    </View>
                  </View>
            ...
            <View>
              <Text className="text-[10px] font-headline font-extrabold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 mb-4 px-2">Settings</Text>
              <View className="bg-white dark:bg-slate-800 rounded-3xl overflow-hidden shadow-sm border border-slate-100 dark:border-slate-700">
                <TouchableOpacity 
                ...
                >
                  <View className="flex-row items-center gap-4">
                    ...
                    <View>
                      <Text className="text-sm font-bold text-slate-900 dark:text-white">Account Security</Text>
                      <Text className="text-[10px] text-slate-400 dark:text-slate-500 font-medium uppercase tracking-wide">2FA Enabled • Password Changed</Text>
                    </View>
                  </View>
                  <ChevronRight size={20} color="#94a3b8" />
                </TouchableOpacity>
      ...
      >
        <View className="gap-4 w-full mt-4">
          {[
            { title: "Daily Occupancy Summary", size: "Generated Live", type: "CSV", icon: BarChart3 },
            { title: "Staff Performance Audit", size: "Generated Live", type: "CSV", icon: FileText },
          ].map((report, i) => (
            <View key={i} className="bg-slate-50 dark:bg-slate-700 rounded-2xl overflow-hidden border border-slate-100 dark:border-slate-600">
              <View className="p-5 flex-row gap-4 items-center">
                <View className="w-12 h-12 bg-primary/10 rounded-xl flex items-center justify-center">
                  <report.icon size={24} color={isDark ? "#10b981" : "#064e3b"} />
                </View>
                <View className="flex-1">
                  <Text className="text-sm font-bold text-slate-900 dark:text-white">{report.title}</Text>
                  <View className="flex-row items-center gap-3 mt-1">
                    <Text className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">{report.size}</Text>
                    <View className="w-1 h-1 bg-slate-200 dark:bg-slate-600 rounded-full" />
                    <Text className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">{report.type}</Text>
                  </View>
                </View>
              </View>
              ...
              <View className="p-4 flex-row gap-3">
                ...
                >
                  ...
                  <Text className="text-white text-xs font-bold">{isDownloading ? 'Generating...' : `Download ${report.type}`}</Text>
                </TouchableOpacity>
                <TouchableOpacity className="flex-1 flex-row items-center justify-center gap-2 py-2.5 px-4 bg-white dark:bg-slate-800 rounded-lg border border-primary/10 dark:border-slate-700 active:opacity-80">
                  ...
                  <Text className="text-primary dark:text-emerald-500 text-xs font-bold">View Report</Text>
                </TouchableOpacity>
              </View>
            </View>
          ))}
      ...
      >
        <View className="gap-6 mt-4 w-full">
          <View className="bg-slate-50 dark:bg-slate-700 rounded-2xl overflow-hidden border border-slate-100 dark:border-slate-600">
            <TouchableOpacity className="p-6 flex-row items-center justify-between active:bg-white dark:active:bg-slate-600">
              <View className="flex-row items-center gap-5">
                ...
                <View>
                  <Text className="font-bold text-slate-900 dark:text-white">Update Password</Text>
                  <Text className="text-xs text-slate-500 dark:text-slate-400 mt-1">Last changed 3 months ago</Text>
                </View>
              </View>
              <ChevronRight size={20} color="#94a3b8" />
            </TouchableOpacity>
          ...
          <View className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl p-6 shadow-sm">
            <View className="flex-row items-center justify-between">
              <View className="flex-row items-center gap-5">
                ...
                <View>
                  <Text className="font-bold text-slate-900 dark:text-white">Two-Factor Auth</Text>
                  <Text className="text-xs text-slate-500 dark:text-slate-400 mt-1">Add an extra layer of security</Text>
                </View>
              </View>
              <Switch trackColor={{ true: '#064e3b', false: '#e2e8f0' }} value={true} />
            </View>
          </View>

          <View className="pt-4">
            <View className="flex-row items-center justify-between mb-4">
              <Text className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">Recent Login History</Text>
              <TouchableOpacity>
                <Text className="text-[10px] font-bold uppercase tracking-widest text-primary">View All</Text>
              </TouchableOpacity>
            </View>
            <View className="gap-3">
              <View className="flex-row items-center justify-between p-4 bg-slate-50 dark:bg-slate-700 rounded-xl border border-slate-100 dark:border-slate-600">
                <View className="flex-row items-center gap-3">
                  <Smartphone size={20} color={isDark ? "#10b981" : "#064e3b"} />
                  <View>
                    <Text className="text-sm font-bold text-slate-900 dark:text-white">MacBook Pro 16"</Text>
                    <Text className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">London, UK • Active Now</Text>
                  </View>
                </View>
          ...
          <TouchableOpacity className="w-full py-4 items-center justify-center border-2 border-red-100 dark:border-red-900/30 rounded-2xl active:bg-red-50 dark:active:bg-red-900/10 mt-4">
            <Text className="text-sm font-bold uppercase tracking-widest text-red-600">Sign Out From All Devices</Text>
          </TouchableOpacity>
        </View>
      </BottomSheet>
